...
const AdminAdd = () => {
    ...
    useEffect(() => {
        document.title = `Admin - Thêm admin`;
    }, []);
    const handleInput = (e) => {    
    ...
    const handleSubmit = (e) => {
        ...
        if (isValid) {
            ...
            axiosClient.post(`Users/register-admin?password=${password}`, admin)
                .then((res) => {
                    if (file) {
                        const formData = new FormData();
                        formData.append("file", file);
                        axios.post(`https://localhost:7164/api/Users/upload/${admin.userName}`, formData, {
                            headers: {
        ...
        <Container>
            <Form onSubmit={handleSubmit}>
                        ...
                        <Row className="mb-3">
                            ...
                            <Form.Group as={Col} >
                                ...
                                <Form.Check
                                    ...
                                    name="gender"
                                    value={true}
                                    checked={admin.gender === 'Nam'} // Sửa ở đây
                                    onChange={handleInput}
                                />
                                <Form.Check
                                    type="radio"
                                    ...
                                    name="gender"
                                    value={false}
                                    checked={admin.gender === 'Nữ'} // Sửa ở đây
                                    onChange={handleInput}
                                />
                ...
                <Button variant="success" type="submit" disabled={isSubmitting}>
                    {isSubmitting ? "Đã tạo người dùng..." : "Gửi"}
                </Button>
            </Form>
        </Container>
