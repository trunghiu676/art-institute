...
const StudentList = () => {
    ...
    const showConfirmation = (student) => {
        handleShowDelete();
        setStudentToDelete(student);
    }
    const handleDelete = () => {
        axiosClient.put(`/Users/Delete/${studentToDelete.id}`)
            .then(res => {
                const updatedCompetitions = students.filter(c => c.id !== studentToDelete.id);
                setStudents(updatedCompetitions);
    ...
    useEffect(() => {
        axiosClient.get(`Users/GetUserByStudent`)
            .then(res => {
                const data = res.data.filter(item => item.status === true);
                setStudents(data);
                setLoading(false);
            })
    }, [studentToDelete, showDelete]);
    ...
    return (<Container>
        <Link to={`/admin/student/add`} className="btn btn-success" >
            Thêm sinh viên
        </Link>
        ...
        <Modal show={showDelete} onHide={handleCloseDelete}>
            ...
            <Modal.Body>Bạn có chắc muốn xóa sinh viên {studentToDelete.fullName}!</Modal.Body>
            <Modal.Footer>
